...
/// Per-choice rooms published by a prefill worker that drew its own rooms for
/// an `n > 1` request (the worker owns room generation whenever the frontend
/// could not draw them upfront). A malformed list is dropped rather than
/// forwarded: decode then rejects the request instead of pairing wrong rooms.
    ...
    Sel: WorkerSelector<ModelRuntimeConfig> + Send + 'static,
{
    ...
    fn prepare_prefill_dispatch(
        ...
        // SGLang cannot run parallel sampling across a PD handoff, so the
        // worker fans an `n > 1` request out into `n` single-sample
        // sub-requests. Each needs its own room, drawn here because only the
        // router knows the `room % dp_size == dp_rank` invariant to keep.
...
/// Draw `count` distinct bootstrap rooms that all satisfy the
/// `room % dp_size == dp_rank` invariant (see [`compute_bootstrap_room`]).
/// `random_room` supplies the raw 63-bit randomness for each draw.
...
mod tests {
    ...
    fn draw_bootstrap_rooms_yields_one_room_for_single_sample() {
        let mut draws = 0;
        let rooms = draw_bootstrap_rooms(Some(3), Some(8), 1, || {
            draws += 1;
            1_000 + draws
        });
        ...
        assert_eq!(draws, 1);
        // `n == 0` is treated like `n == 1` rather than producing no rooms.
        assert_eq!(draw_bootstrap_rooms(None, None, 0, || 42), vec![42]);
    }
