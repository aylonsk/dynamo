...
class DecodeWorkerHandler(BaseWorkerHandler):
    ...
    async def generate(
        ...
        if self.serving_mode == DisaggregationMode.DECODE:
            ...
            # SGLang cannot pair parallel samples across a PD handoff, so an
            # n > 1 request runs as n single-sample sub-requests, one room
            # each, merged back by choice index below (see
            # dynamo.sglang.parallel_sampling). An older frontend supplies a
            # single room for n > 1 and is rejected with HTTP 400 here.
            ...
            # A single choice keeps the pre-fan-out stream shape untouched:
            # SGLang's own indices and response id pass straight through.
            if num_choices == 1:
                choice_indices: list[int | None] = [None]
                response_id = None
            else:
                choice_indices = list(range(num_choices))
                response_id = trace_id or context.id()
            choice_streams: list[AsyncIterator[Dict[str, Any]]] = []
            for choice_index, decode in zip(choice_indices, decode_streams):
                if not self.use_sglang_tokenizer:
            ...
            if num_choices == 1:
                ...
                async for out in self._merge_choice_outputs(choice_streams):
                    yield out
        else:
            raise_if_unextracted_multimodal(request)
    ...
    async def _merge_choice_outputs(
        ...
        Each sub-stream already stamps its own choice index. Only the usage
        block needs fixing up: a sub-stream reports its own completion count,
        while the single-stream n > 1 path reports the request-wide total in
        every finishing chunk, so the same running total is restored here.
        """
        completion_tokens_per_choice: dict[int, int] = {}
        async for choice_index, out in merge_choice_streams(choice_streams):
